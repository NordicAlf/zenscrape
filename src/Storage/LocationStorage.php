<?php
declare(strict_types=1);

namespace Zenscrape\Storage;

class LocationStorage
{
    // North America
    const COMMON_NORTH_AMERICA = 'na'; // any available country in North America
    const ANGUILLA = 'ai';
    const ANTIGUA_AND_BARBUDA = 'ag';
    const UNITED_STATES = 'us';
    const ARUBA = 'aw';
    const BAHAMAS = 'bs';
    const BARBADOS = 'bb';
    const BELIZE = 'bz';
    const BERMUDA = 'bm';
    const BONAIRE_SAINT_EUSTATIUS_AND_SABA = 'bq';
    const BRITISH_VIRGIN_ISLANDS = 'vs';
    const CANADA = 'ca';
    const CAYMAN_ISLANDS = 'ky';
    const COSTA_RICA = 'cr';
    const CUBA = 'cu';
    const CURACAO = 'cw';
    const DOMIMICA = 'dm';
    const DOMINICAN_REPUBLIC = 'do';
    const EL_SALVADOR = 'sv';
    const GRENADA = 'gd';
    const GUADELOUPE = 'gp';
    const GUATEMALA = 'gt';
    const HAITI = 'ht';
    const HONDURAS = 'hn';
    const JAMAICA = 'jm';
    const MATINIQUE = 'mq';
    const MEXICO = 'mx';
    const NICARAGUA = 'ni';
    const PANAMA = 'pa';
    const PURTO_RICO = 'pr';
    const SAINT_BARTHELEMY = 'bl';
    const SAINT_KITTS_AND_NEVIS = 'kn';
    const SAINT_LUCIA = 'lc';
    const SAINT_MARTIN = 'mf';
    const SAINT_PIERRE_AND_MUQUELON = 'pm';
    const SAINT_VINCENT_AND_THE_GRENADINES = 'vc';
    const SING_MAARTEN = 'sx';
    const TRINIDAD_AND_TOBAGO = 'tt';
    const TURKSAND_CAICOS_ISLANDS = 'tc';
    const UNITED_STATES_VIRGIN_ISLANDS = 'vi';

    // Europe
    const COMMON_EUROPE = 'eu'; // any available country in Europe
    const ALAND_ISLANDS = 'ax';
    const ALBANIA = 'al';
    const ANDORRA = 'ad';
    const AUSTRIA = 'at';
    const BELARUS = 'by';
    const BELGIUM = 'be';
    const BOSHIA_AND_HERZQGOVINA = 'ba';
    const BULGARIA = 'bg';
    const CROATIA = 'hr';
    const CYPRUS = 'cy';
    const CZECH_REPUBLIC = 'cz';
    const DENMARK = 'dk';
    const ESTONIA = 'ee';
    const FAROE_ISLANDS = 'fo';
    const FINLAND = 'fi';
    const FRANCE = 'fr';
    const GERMANY = 'de';
    const GIBRALTAR = 'gi';
    const GREECE = 'gr';
    const GREENLAND = 'gl';
    const GUERNSEY = 'gg';
    const HUNGARY = 'hu';
    const ICELAND = 'is';
    const IRELAND = 'ie';
    const ISLE_OF_MAN = 'im';
    const ITALY = 'it';
    const JERSEY = 'je';
    const KOSOVO = 'xk';
    const LATVIA = 'lv';
    const LIECHTENSTEIN = 'li';
    const LITHUANIA = 'lt';
    const LUXEMBOURG = 'lu';
    const MACEDONIA = 'mk';
    const MALTA = 'mt';
    const MOLDOVA = 'md';
    const MONACO = 'mc';
    const MONTENEGRO = 'me';
    const NETHERLANDS = 'nl';
    const NORWAY = 'no';
    const POLAND = 'pl';
    const PORTUGAL = 'pt';
    const ROMANIA = 'ro';
    const SAN_MARINO = 'sm';
    const SERBIA = 'rs';
    const SLOVAKIA = 'sk';
    const SLOVENIA = 'si';
    const SPAIN = 'es';
    const SWEDEN = 'se';
    const SWITZERLAND = 'ch';
    const TURKEY = 'tr';
    const UKRAINE = 'ua';
    const UNITED_KINGDOM = 'uk';
    const VATICAN = 'va';
}